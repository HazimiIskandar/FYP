...
export default function CaregiverEditSeniorMenuScreen({
  ...
  // Rule 3 front-end: caregiver-side "Remove Senior from My Caregiver
  // Account" affordance. Two states drive the UX — the inline-row
  // confirm modal is open while the caregiver is reviewing the
  // destructive action; `removingSenior` flips true during the DELETE
  // network round-trip so the button disables and avoids
  // double-submits.
  ...
  // App.js, which flips the senior to restricted Home. We do NOT
  // clear the Senior_Link_Code row — a new caregiver / the same
  // caregiver can re-link the senior using the same 6-digit code.
...
const styles = StyleSheet.create({
  ...
  // benign Settings rows above. Light red border + red chevron
  // reinforces the destructive intent even for first-time users.
  unlinkRow: {
    backgroundColor: '#FFFFFF',
    borderRadius: 16,
    borderWidth: 1,
