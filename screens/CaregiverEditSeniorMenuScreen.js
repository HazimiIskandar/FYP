...
export default function CaregiverEditSeniorMenuScreen({
  ...
  // Rule 3 — caregiver removes senior. Invokes the backend's
  // DELETE /seniors/:senior_id/caregivers/:caregiver_id endpoint,
  // which removes the Senior_has_Caregiver junction row (and the
  // senior's other data remains untouched per Rule 5). Then
  // re-runs onRefresh so the caregiver's roster reflects the
  // removal immediately. The senior app picks up the change via
  // its own 30s link-polling interval (App.js) or on next app
  // foreground. We deliberately do NOT clear the senior's link
  // code from Senior_Link_Code — a new caregiver / the same
  // caregiver can re-link the senior using the same code.
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        {/* Rule 3 — caregiver removes senior from their caregiver
            ...
            doesn't lose the linkage for an active senior; the senior's
            own data (points, streaks, check-ins, etc.) is retained per
            Rule 5 because the DELETE only removes the Senior_has_Caregiver
            junction row. */}
        <TouchableOpacity
