        ...
        if new_state != current_state:
            if new_state == "Emergency":
                ...
                if vital_name == "Heart Rate":
                    log_vital_alert_to_telegram(f"EMERGENCY ALERT: {vital_name} CRITICAL ({value} BPM) detected in Living Room!")
                    log_vital_alert_to_telegram(f"Please send emergency help immediately!")
                else:
                    log_vital_alert_to_telegram(f"EMERGENCY ALERT: {vital_name} CRITICAL ({value} breaths/min) detected in Living Room!")
                    log_vital_alert_to_telegram(f"Please send emergency help immediately!")
